...
- **运行模式**：账户模式（默认）与**免校验模式**（`VL_EASE=true`）。
  后者没有 Key、账号、配额与管理接口，见 [§2.8](#28-免校验模式vl_easetrue)。
...
| `quota_exhausted` | 429 | 配额不足（见 §2.2） |
| `timeout` | 504 | 上游超时 |
...
| 根路径 `/` | 纯文本导航页（需 Key） | **图形化解析页**（`text/html`） |
...
| 参数 | 必填 | 说明 |
| --- | --- | --- |
| `url` | 二选一 | 分享链接或整段分享文案（自动从文本里提取链接） |
...
| `/readyz` | 就绪探针 |
| `/` | 根路径：免校验模式返回**图形化解析页**（`text/html`，内嵌、无外部资源）；账户模式返回纯文本导航页（需 Key） |
